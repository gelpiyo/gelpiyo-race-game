...
function gen5(){
  const p=[],R=550,amp=90,n=11,sx=1.3,sy=0.85;
  for(let i=0;i<1400;i++){const t=(i/1400)*Math.PI*2;const r=R+amp*Math.sin(n*t);p.push({x:CX+r*Math.cos(t)*sx,y:CY+r*Math.sin(t)*sy});}
  return p;
}
function gen6(){
  const p=[],R=550,a1=100,f1=9,a2=40,f2=3;
  for(let i=0;i<1400;i++){const t=(i/1400)*Math.PI*2;const r=R+a1*Math.sin(f1*t)+a2*Math.sin(f2*t+0.5);p.push({x:CX+r*Math.cos(t),y:CY+r*Math.sin(t)});}
  return p;
}
...
console.log('=== DEBUG v6 (updated generators) ===');
check('5. NEON SPIRAL (oval 11-bump)',gen5(),200);
